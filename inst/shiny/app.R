...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    width = "500",
    fluidRow(
      actionButton("smilesButton", "Smiles", width = 100, class = "btn m-1"),
      actionButton("molFileButton", "MOL File", width = 100, class = "btn m-1"),
      actionButton("jmeFileButton", "JME File", width = 100, class = "btn m-1")
    ),
    textInput("smilesTextInput", "Smiles", width = "100%"),
    textAreaInput("molFileTextInput", label = "MOL File"),
    textInput("jmeFileTextInput", "JME string", width = "100%"),
    fluidRow(
      actionButton("readSampleMoleculeButton", "Read Molecule",
                   width = 125, class = "btn m-1"),
      actionButton("readSampleMultipartStructureButton", "Read Multipart Structure",
                   width = 150, class = "btn m-1"),
      actionButton("readSampleReactionButton", "Read Reaction",
                   width = 125, class = "btn m-1")
    ),
    fluidRow(
      actionButton("useJmeButton", "Use JME", width = 150, class = "btn m-1"),
      actionButton("useMolButton", "Use MOL file", width = 150, class = "btn m-1"),
    ),
    fluidRow(
      actionButton("clearEditorButton", "Clear Editor", width = 150, class = "btn m-1"),
      actionButton("clearFieldsButton", "Clear Fields", width = 150, class = "btn m-1")
    )
  ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$readSampleMultipartStructureButton,
               useJME(session, "jsmeElement", "9 9 C 6.68 -7.15 C 5.47 -6.45 C 4.26 -7.15 C 4.26 -8.55 C 5.47 -9.25 C 6.68 -8.55 C 5.47 -5.05 O- 6.68 -4.35 O 4.26 -4.35 1 2 1 2 3 2 3 4 1 4 5 2 5 6 1 6 1 2 2 7 1 7 8 1 7 9 2|1 0 Na+ 12.21 -6.61"))

  observeEvent(input$readSampleReactionButton,
               useJME(session, "jsmeElement", "3 2 C:1 1.41 -7.12 O:2 1.41 -5.72 Cl 2.63 -7.82 1 2 2 1 3 1|3 2 N:3 5.72 -6.78 C:4 7.12 -6.78 H:5 5.02 -7.99 1 2 1 1 3 1 >> 5 4 C:1 13.51 -6.40 O:2 13.51 -5.00 N:3 14.72 -7.10 C:4 15.94 -6.40 H:5 14.71 -8.50 1 2 2 1 3 1 3 4 1 3 5 1"))
